...
public class JMSElasticProducer {
	...
	public void init() {
		...
		try {
			...
			Queue queue = session.createQueue("elastic-delete-queue");
	...
	public void receiveElasticDataAndPerformIndexing(MessageConsumer consumer) throws JMSException, InterruptedException {
		...
		consumer.setMessageListener(new MessageListener() {
			...
			public void onMessage(Message message) {
				System.out.println("Listening...to Delete Queue");
				...
				try {
					...
					// Indexing the notes
					ElasticSearchUtility.deleteNoteFromElastic(note);
					System.out.println("Deleted the note from the elastic saerch");
